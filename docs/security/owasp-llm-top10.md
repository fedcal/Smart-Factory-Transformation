...
plan: 05
type: owasp-llm-mapping
standard: OWASP LLM Top 10 (2025)
closes: AR-06
created: 2026-05-25
---

# OWASP LLM Top 10 — Mapping a Mitigazioni Concrete (SEC-02)
...
Chiude **AR-06** (10-SECURITY.md): OWASP LLM hardening assegnato a Phase 11.
...
| Checkpoint | Executor GSD blocca install di pacchetti non verificati (gate `blocking-human`) | Policy GSD executor (checkpoint protocol) |
...
| Layer | Mitigazione | File:funzione |
|-------|------------|---------------|
| Angular auto-escape | Template Angular: interpolazione `{{ expr }}` auto-escaped; nessun `innerHTML` non sanitizzato | `apps/factory-ui/src/app/shared/alert-feed/alert-feed.component.ts` (10-SECURITY.md T-10-07-02) |
...
| SQL params | Tutte le query usano `$1..$N` parametrizzati; nessun f-string in SQL | `apps/api-gateway/src/svc_api_gateway/routers/kpi.py` (10-SECURITY.md T-10-02-01) |
...
| Layer | Mitigazione | File:funzione |
|-------|------------|---------------|
| No secrets in prompts | I prompt di sistema non contengono credenziali o chiavi API; solo istruzioni operative | Convention documentata + review (10-SECURITY.md T-10-01-04) |
...
Vedere `docs/security/rate-limit-scaling.md` per il path evolutivo Redis.
...
Questo documento chiude **AR-06** (10-SECURITY.md): OWASP LLM Top-10 hardening Phase 11.
