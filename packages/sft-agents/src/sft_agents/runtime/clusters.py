...
#: Default fallback target for the supply/SCM router (Plan 09-01 planner decision).
#: CostAnalyzer is autonomous / read-only (D-SCM-AUTO) — no HITL, no irreversible
#: side effects. Unknown-target routing to an autonomous agent is the safest fallback.
...
def build_supply_subgraph(
    ...
    Structural mirror of :func:`build_knowledge_subgraph` (D-X-04 gateway pattern).
    Each entry in ``child_callables`` is the supply-agent's async ``__call__``
    (already a coroutine that returns a state-delta dict). The graph wires:
    ...
    ``_route(state)`` reads ``state.get("target_agent")``. If the value is
    missing or not present in ``child_callables``, the router falls back to
    ``cost-analyzer`` (autonomous, read-only, D-SCM-AUTO) and emits a
    ``scm_route_unknown_target`` structlog warning so observability still surfaces
    the routing-table drift.

    Parameters
    ----------
    child_callables:
        Mapping of agent slug (kebab-case, e.g. ``inventory-manager``,
        ``energy-optimizer``, ``cost-analyzer``, ``demand-forecaster``)
        to the async callable that implements the agent's node body. MUST include
        ``cost-analyzer`` (the fallback) — otherwise the fallback would
        silently fail at run-time. Non-empty.
    ...
    ------
    ValueError
        If ``child_callables`` is empty or does not contain ``cost-analyzer``.
    """
    if not child_callables:
        raise ValueError(
            "child_callables must be non-empty for the supply subgraph"
        )
