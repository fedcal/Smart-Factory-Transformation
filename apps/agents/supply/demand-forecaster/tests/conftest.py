"""Conftest for DemandForecaster agent tests (Phase 9, Wave 0 scaffold).
...
Real fixtures (scm.historical_orders query harness, Holt-Winters series helpers) land
with plan 09-05 (demand-forecaster implementation).
"""
...
def make_interrupt_fn():
    ...
    initial node execution that pauses for HITL approval from ProductionPlanner).
    ...
    def factory(resume_payload: dict[str, Any] | None = None) -> Any:
        ...
        _payload = resume_payload or {
            "approved": True,
            "approver_id": "production-planner-001",
        }
